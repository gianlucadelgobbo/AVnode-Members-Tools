    ...
    if (auth) {
      if (req.session.user.role != 'admin' && req.session.user.role != 'superadmin'){
        ...
        DB.settings.findOne({}, function(e, result) {
          if (result) {
            if (!result.emailDispatcher) result.emailDispatcher = require('../../config/config.js')._config.emailDispatcher;
            res.render('settings', { title: __('Settings'), countries : CT, result : result, msg:{}, udata : req.session.user});
          } else {
            if (!global._config.emailDispatcher) global._config.emailDispatcher = require('../../config/config.js')._config.emailDispatcher;
