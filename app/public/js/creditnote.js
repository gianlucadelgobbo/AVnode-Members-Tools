...
$(function() {
	...
	$("#creditnote").submit(function() {
		$(".disabled").removeAttr('disabled');
		return checkCreditnote();
	});
...
function showInvoices() {
	$('.modal-alert .modal-title').html("Invoices");
	...
	$.ajax({
		...
		success: function( data ) {
			...
			for(var a=0;a<data.result.length;a++){
				var d = new Date(data.result[a].invoice_date);
				var date = d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear();
				str+="<a href=\"#\" onclick=\"setInvoice('"+data.result[a]._id+"','"+date+"','"+data.result[a].invoice_number+"');\" class=\"list-group-item\"><h4 class=\"list-group-item-heading\"><span class=\"label label-default\">"+data.result[a].invoice_number+"</span> "+date+" "+data.result[a].to_client.name+"</h4><p class=\"list-group-item-text\">"+data.result[a].description+"</p></a>"
			}
			str+= "</div>";
...
function setInvoice(_id,date,invoice_number) {
	$('#invoice_id').val(_id);
	$('#invoice_number').val(invoice_number);
	$('#invoice_date').val(date);
	$('#invoice_url').attr('href', '/invoice/?id='+_id).removeAttr('disabled');
...
function checkDate() {
	var creditnote_date = $("#creditnote_date");
	var d = creditnote_date.val().split("/");
	$.ajax({
		url: "/api/accounting/creditnotes",
		dataType: "json",
		data: {
			ajax: true,
			creditnote_date:creditnote_date.val(),
			creditnote_number:$("#creditnote_number").val()
		},
		success: function( data ) {
			if (data.result.length) {
				var d = new Date(data.result[data.result.length-1].creditnote_date);
				showModal('error', "La data deve essere uguale o successiva al "+d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear(), function () {setTimeout("\$(\"#creditnote_date\").focus()",50)});
			}
		}
	});
}
