  ...
  DB.db.open(function(e, d){
    ...
    if (e) {
      ...
      DB.db.collection('settings').findOne({}, function(e, o){
        if (!o) o = require('../../config/config.js')._config;
        global._config = o;
        //console.log(o)
...
DB.update_settings = function(newData, userData, callback) {
  DB.settings.findOne({_id:new ObjectID(newData.id)}, function(e, o){
    ...
    DB.settings.findOne({_id:newData._id}, function(e, o){
      global._config = o;
      if (!global._config.roles) global._config.roles = require('../../config/config.js')._config.roles;
      i18nAdmin.setLocale(o.defaultLocale);
      callback(e, o);
    });
  });
...
DB.insert_doc = function(coll, newData, userData, callback) {
  console.log("newData");
  console.log(newData);
  delete newData.id;
  var d = newData.doc_date.split("/");
  newData.doc_date = new Date(Date.UTC(parseInt(d[2]),parseInt(d[1])-1,parseInt(d[0])));
  ...
  if (coll == "purchases") {
    newData.doc_to = global._config.company;
  } else {
    newData.doc_from = global._config.company;
  }
  newData.doc_number = parseInt(newData.doc_number);
  ...
  DB[coll].insert(newData, {safe: true}, function(err, records){
    callback(err, records.ops[0]);
  });
};
DB.update_doc = function(coll, newData, userData, callback) {
  DB[coll].findOne({_id:new ObjectID(newData.id)}, function(e, o){
    newData._id = o._id;
    var d = newData.doc_date.split("/");
    newData.doc_date = new Date(Date.UTC(parseInt(d[2]),parseInt(d[1])-1,parseInt(d[0])));
    ...
    newData.revisions.push({userID : userData._id,username: userData.name,time : new Date()});
    delete newData.id;
    //console.log(newData.doc_date);
    DB[coll].save(newData);
    //DB[coll].findOne({_id:newData._id}, function(e, no){
      callback(e, newData);
    //});
  });
};
DB.delete_doc = function(coll, id, callback) {
  DB[coll].remove({_id: new ObjectID(id)},{safe: true}, function(err, records){
...
DB.insert_creditnote = function(newData, userData, callback) {
  ...
  if(newData.invoice.doc_date!=""){
    d = newData.invoice.doc_date.split("/");
    newData.invoice.doc_date = new Date(Date.UTC(parseInt(d[2]),parseInt(d[1])-1,parseInt(d[0])));
  }
  newData.doc_number = parseInt(newData.doc_number);
  newData.vat_perc = parseInt(newData.vat_perc);
  unformatPrices(newData);
