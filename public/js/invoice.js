	...
	$.ajax({
		...
		success: function( data ) {
			...
			for(var a=0;a<data.result.length;a++){
				...
				str+="<a href=\"#\" onclick=\"setOffer('"+data.result[a]._id+"','"+date+"','"+data.result[a].doc_number+"');\" class=\"list-group-item\"><h4 class=\"list-group-item-heading\"><span class=\"label label-default\">"+data.result[a].doc_number+"</span> "+date+" "+data.result[a].doc_to.name+"</h4><p class=\"list-group-item-text\">"+data.result[a].description+"</p></a>"
			}
			str+= "</div>";
...
function setOffer(_id,date,doc_number) {
	$('#offer_id').val(_id);
	$('#offer_number').val(doc_number);
	$('#offer_date').val(date);
	$('#offer_url').attr('href', '/'+dbName+'/accounting/offers/'+_id).removeAttr('disabled');
