	...
	$.ajax({
		...
		success: function( data ) {
			...
			for(var a=0;a<data.result.length;a++){
				...
				str+="<a href=\"#\" onclick=\"setInvoice('"+data.result[a]._id+"','"+date+"','"+data.result[a].doc_number+"');\" class=\"list-group-item\"><h4 class=\"list-group-item-heading\"><span class=\"label label-default\">"+data.result[a].doc_number+"</span> "+date+" "+data.result[a].doc_to.name+"</h4><p class=\"list-group-item-text\">"+data.result[a].description+"</p></a>"
			}
			str+= "</div>";
...
function setInvoice(_id,date,doc_number) {
	$('#invoice_id').val(_id);
...
function checkDate() {
	var doc_date = $("#doc_date");
	var d = doc_date.val().split("/");
	$.ajax({
		...
		success: function( data ) {
			if (data.result.length) {
				var d = new Date(data.result[data.result.length-1].doc_date);
				showModal('error', "La data deve essere uguale o successiva al "+d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear(), function () {setTimeout("\$(\"#doc_date\").focus()",50)});
			}
		}
	});
}
